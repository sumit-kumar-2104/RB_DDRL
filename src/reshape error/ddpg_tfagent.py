...
class DDPG(TD3):
    ...
    :param exploration_strategy: Custom exploration strategy
    ...
    def get_eval_episode_rewards(self):
        return self.eval_episode_rewards

    def get_exploration_eps(self):
        if self.exploration_strategy is not None:
            return self.exploration_strategy.eps
        else:
            return None

    def evaluate_policy(self, n_eval_episodes=10, render=False):
        """
        Evaluate the agent in the given environment.

        :param n_eval_episodes: Number of episodes to evaluate the agent
        :param render: Whether to render the environment
        :return: Mean reward for the evaluation episodes
        """
        if self.eval_env is None:
            raise ValueError("No evaluation environment provided.")
        ...
        self.eval_episode_rewards = eval_episode_rewards
