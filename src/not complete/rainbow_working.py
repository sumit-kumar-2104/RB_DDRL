...
import math
import logging
from rb_env import ClusterEnv  # Make sure this is correctly defined
...
class RainbowQNetwork(network.Network):
    ...
    def call(self, inputs, step_type=None, network_state=(), training=False):
        x = inputs # Ensure inputs are float32
        x = self.flatten(x)
        ...
        q_values = tf.reduce_sum(q_values, axis=-1, keepdims=False)
...
train_rainbow_dqn(num_iterations=20000)
