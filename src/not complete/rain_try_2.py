...
def collect_step(environment, policy, replay_buffer):
    ...
    action_step = tf.nest.map_structure(lambda t: tf.squeeze(t, axis=0), action_step)
    next_time_step = tf.nest.map_structure(lambda t: tf.squeeze(t, axis=0), next_time_step)
...
# Helper function to ensure correct tensor shapes
def check_shape(tensor, expected_shape):
    if tensor.shape != expected_shape:
        raise ValueError(f"Unexpected shape: {tensor.shape}, expected: {expected_shape}")
...
class TFPrioritizedReplayBuffer(tf_uniform_replay_buffer.TFUniformReplayBuffer):
    ...
    def add_batch(self, items):
        ...
        # Update priorities to the maximum priority for the new batch
        max_priority = tf.reduce_max(self._priorities)
        self._priorities[self._index].assign(max_priority if max_priority > 0 else 1.0)
...
def train_dqn(
    ...
    try:
        ...
        print(f"Error in dataset and iterator creation: {e}")
    ...
    for iteration in range(num_iterations):
        ...
        for _ in range(collect_steps_per_iteration):
            try:
                ...
                logging.error(f"Error in collect_step: {e}")
                continue
        ...
        try:
            ...
            tf.print("Sampled experience shapes:", {k: tf.shape(v) for k, v in experience.items()})
